...
final class AdminController
{
    ...
    public function tables(Request $request, string $workspaceId): JsonResponse
    {
        ...
        if ($request->filled('baseId')) $query->where('t.base_id', $request->validate(['baseId' => ['uuid']])['baseId']);
    ...
    private function admin(Request $request): void
    {
        ...
        if (! $profile?->can_manage_users || $profile->disabled_at) throw ApiException::forbidden('Admin access required');
    }
}
